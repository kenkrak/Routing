...
public class MapGraph {
	...
	/** Calculate Direct distance and set the ActualDistance from node 1 to node 2
	 *
	 * @param currNode The current node from which distance to goal has to be calculated
	 * @param endNode the goal node
	 *
	 */
	public void setActualDist(MapNode currNode, MapNode endNode) {
		GeographicPoint startPoint = currNode.getLocation();
		GeographicPoint endPoint = endNode.getLocation();
		currNode.setActualDistance(startPoint.distance(endPoint));
	}
										  ...
										  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		int countNumTimes=0;
		PriorityQueue<MapNode> minWeightEdge = new PriorityQueue<MapNode>();
		//Queue<GeographicPoint> toBeTraversed = new LinkedList<GeographicPoint>();
		HashMap<MapNode,MapNode> parentMap = new HashMap<MapNode,MapNode>();
		Set<MapNode> traversed = new HashSet<MapNode>();
		double currDist=0.0;

		minWeightEdge.add(pointNodeMap.get(start));
		pointNodeMap.get(start).setDistance(0.0);
		//System.out.println("The goal is " + pointNodeMap.get(goal));

		while(!minWeightEdge.isEmpty()) {
			MapNode currNode = minWeightEdge.peek();
			if(traversed.contains(currNode)) {
				minWeightEdge.remove();
				continue;
			}
			traversed.add(currNode);
			//System.out.println(currNode.getDistance());
			if(currNode == pointNodeMap.get(goal)) {
				System.out.println(++countNumTimes);
				List<GeographicPoint> shortestWeightPath = reconstructPath(parentMap, pointNodeMap.get(start), pointNodeMap.get(goal));
				return shortestWeightPath;
			}
			currDist=currNode.getDistance();
			nodeSearched.accept(currNode.getLocation());
			++countNumTimes;

			minWeightEdge.remove();
			Set<MapEdge> adjacentEdges = currNode.getEdges();
			for(MapEdge e: adjacentEdges) {
				double edgeLength = e.getLength();
				if((currDist + edgeLength) < e.getEndNode().getDistance()) {
					e.getEndNode().setDistance(currDist + edgeLength);
				// get edge length
					minWeightEdge.add(e.getEndNode());
				    parentMap.put(e.getEndNode(), currNode);
											 ...
											 GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		minWeightEdge.add(pointNodeMap.get(start));
		pointNodeMap.get(start).setDistance(0.0);
		pointNodeMap.get(start).setDistanceFromStart(0.0);
		//System.out.println("The goal is " + pointNodeMap.get(goal));
		...
		while(!minWeightEdge.isEmpty()) {
			...
			currDist = currNode.getDistanceFromStart();
			nodeSearched.accept(currNode.getLocation());
			...
			for(MapEdge e: adjacentEdges) {
				double edgeLength = e.getLength();
				setActualDist(e.getEndNode(), pointNodeMap.get(goal));
				double actualDist = e.getEndNode().getActualDistance();
				double currDistValue = e.getEndNode().getDistance();
				if((currDist + edgeLength) < e.getEndNode().getDistanceFromStart()) {
					e.getEndNode().setDistanceFromStart(currDist + edgeLength);
					//System.out.println("setting actual distance  " + e.getEndNode().getDistanceFromStart());
					parentMap.put(e.getEndNode(), currNode);
					if(e.getEndNode().getDistanceFromStart() + actualDist< currDistValue) {
						e.getEndNode().setDistance(e.getEndNode().getDistanceFromStart() + actualDist);
						//System.out.println("setting distance" + e.getEndNode().getDistance());
				    }
					minWeightEdge.add(e.getEndNode());
				}
...
//
//		List<GeographicPoint> route = theMap1.bfs(new GeographicPoint(1.0,1.0),
...
//
//		List<GeographicPoint> route2 = theMap.dijkstra(start1,end1);
//		for(int i=0;i<route.size();i++) {
//			System.out.println(route2.get(i));
//		}
//		List<GeographicPoint> route2 = theMap.aStarSearch(start,end);
//		System.out.println(route2);
//		PriorityQueue<MapNode> minWeightEdge = new PriorityQueue<MapNode>();
//		MapNode n1 = new MapNode(start);
//		MapNode n2 = new MapNode(end);
//		MapNode n3 = new MapNode(start1);
//		MapNode n4 = new MapNode(end1);

//		n1.setDistance(10.9);
//		n2.setDistance(100.0);
//		n3.setDistance(45.0);
//		n4.setDistance(88.2);
//		minWeightEdge.add(n1);
//		minWeightEdge.add(n2);
//		minWeightEdge.add(n3);
//		minWeightEdge.add(n4);
//		System.out.println(minWeightEdge.peek() + " " + minWeightEdge.peek().getDistance());
//		minWeightEdge.remove();
//		System.out.println(minWeightEdge.peek()+ " " + minWeightEdge.peek().getDistance());
//		minWeightEdge.remove();
//		System.out.println(minWeightEdge.peek()+ " " + minWeightEdge.peek().getDistance());
